...
void pack_gaussians3d(const Gaussians3D& g, std::vector<float>& means, std::vector<float>& covs,
  ...
  // Caller (gaussian_splat_3d_forward) fills view-dependent RGB via fill_colors_from_sh when
  // sh_coeffs are present. Pack those RGB colors for Metal rasterization.
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int r = 0; r < 3; ++r) {
      for (int col = 0; col < 3; ++col) {
        covs[static_cast<size_t>(i) * 9 + r * 3 + col] =
            g.covariances[static_cast<size_t>(i)].m[r][col];
      }
    }
    for (int ch = 0; ch < c; ++ch) {
      colors[static_cast<size_t>(i) * c + ch] =
          g.colors.empty() ? 0.0f : g.colors[static_cast<size_t>(i)][ch];
    }
...
bool metal_device_available() { return metal::metal_available(); }
...
Image gaussian_splat_3d_forward_metal_impl(const Gaussians3D& gaussians,
  ...
  metal::Splat3DMetalOptions metal_opts;
  metal_opts.near_plane = opts.near_plane;
  metal_opts.min_covariance = opts.min_covariance;
  metal_opts.sigma_radius = opts.sigma_radius;
  metal_opts.compact_box_beta = opts.compact_box_beta;
  metal_opts.use_compact_box = opts.use_compact_box;

  if (!metal::gaussian_splat_3d_forward(means.data(), covs.data(), colors.data(), opacities.data(),
                                        static_cast<int>(gaussians.means.size()), c, intr, c2w,
                                        height, width, image.data(), metal_opts)) {
    return Image();
  }
  return image;
}
...
GradientsProjected2D gaussian_splat_3d_projected_backward_metal_impl(
  ...
  if (!metal::gaussian_splat_3d_projected_backward(
          ...
          grad_opacities.data(), metal_opts)) {
    return grads;
  }
